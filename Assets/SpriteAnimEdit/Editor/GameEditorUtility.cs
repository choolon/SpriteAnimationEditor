...
namespace SpriteAnimEdit
{
    ...
    public class GameEditorUtility
    {
        ...
        public static string CreateNewPrefab(string name)
        {
		...
		/// <param name="leftTopCoordinate">是否转换结果的坐标系（转为左上角为0,0坐标）</param>
        /// <returns></returns>
		public static Rect GetTextureBound(Texture2D tex, bool leftTopCoordinate=false)
        {
            //(0,0)在tex的左下角
			...
			if(leftTopCoordinate)
				return new Rect(xmin, tex.height - ymin, xmax - xmin + 1, ymax - ymin + 1);
			else
        ...
        {
            Color[] newColors = new Color[(int)rect.width * (int)rect.height];
            int index = 0;
            Color32[] pixels = tex.GetPixels32();
            for (int y = (int)rect.y; y <= (rect.y + rect.height - 1); y++)
            {
                for (int x = (int)rect.x; x <= (rect.x + rect.width - 1); x++)
                {
