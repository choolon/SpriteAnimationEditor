...
namespace SpriteAnimEdit.Editor.Sprite
{
    public class SpriteAtlasEditorSettingView
    {
        ...
        private void DrawTextureSettings()
        {
			...
			_spriteAtlasProxy.outputDebugGraphic = EditorGUILayout.Toggle("输出调试图形", _spriteAtlasProxy.outputDebugGraphic);
			_spriteAtlasProxy.anchorPointInEditor = EditorGUILayout.Vector2Field("编辑器中的锚点位置（相对下部中间）", _spriteAtlasProxy.anchorPointInEditor);
			_spriteAtlasProxy.anchorPointInImage = EditorGUILayout.Vector2Field("图片上的锚点位置（相对下部中间）", _spriteAtlasProxy.anchorPointInImage);
